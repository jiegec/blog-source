...
4. 而加载到内存里的时候，就是直接大段地连续地加载到内存中，所以可以提前计算好各个部分的地址。例如要把 ELF 加载到 0x400000，那就把 file header 和 program header 放在开头，然后因为 section 需要对齐，例如对齐到 0x1000，那就把第一个 section 放到 0x401000，往后再对齐再放下一个 section，依此类推，直到把所有 section 都放下为止。
