...
而 Gracemont 和 Golden Cove 的局部预测器的设计就不太一样，对比如下：

|                      | Gracemont | Golden Cove 局部预测器          |
|----------------------|-----------|---------------------------------|
| 容量                 | 26        | 512                             |
| tag 位数             | 16        | N/A，用 PC[8:0] 直接映射         |
| ctr 位数             | 2         | 4                               |
| 新表项 ctr 取值      | 0         | N/A，表项总是存在                |
| 不命中时预测为       | 无依赖    | N/A，总是命中                    |
| 命中时预测为         | 有依赖    | ctr==15 时预测无依赖，否则有依赖 |
| 命中时有依赖         | ctr=0     | ctr=0                           |
| 命中时无依赖         | ctr+=1    | ctr+=1                          |
| ctr 等于多少时 evict | 3         | N/A，不会 evict                  |

此外 Golden Cove 还有全局预测器来覆盖局部预测器的预测结果，详见 [Golden Cove](./intel-golden-cove.md) 的逆向结果。
