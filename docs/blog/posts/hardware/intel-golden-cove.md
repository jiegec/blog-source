...
测试时，让这对 Store-Load 采用相同/不同的地址进行访存，具体地，首先是 100 次相同地址，然后 20 次不同地址，最后 10 次相同地址，每次执行的周期数如下：
...
接下来，参考论文中的方法，生成几次错误预测，看看多少次由错误预测导致的回滚（即预测为无依赖，但实际上有依赖，导致了回滚；另一方面，如果预测为有依赖，实际上没有，并不会导致回滚）后，全局预测器会介入，强制预测所有 Load 和先前的 Store 有依赖关系。用 10 个 Store-Load 对，先让这些 Load 都被局部预测为无依赖，再连续 10 次有依赖地按顺序执行这 10 个 Load，结果如下：
